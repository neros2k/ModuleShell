...
public abstract class CommandListener extends AbstractListener {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        this.onCommand(new CommandContext(event.getJDA(), event.getMember(), event.getCommandString()));
    }
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        if(split[0].equals(this.COMMAND_NAME)) {
            this.onCommand(new CommandContext(event.getJDA(), event.getMember(), message));
        }
    }

    abstract void onCommand(CommandContext ctx);
